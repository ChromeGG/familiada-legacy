...
// see original map: https://gist.github.com/arifmahmudrana/b991f1c15162654b3a481780884c0d4d
const getMessages = (t: Translate) => {
  return {
    ...
    'number.base': t`number.base`,
    'number.greater': '{{#label}} must be greater than {{#limit}}',
    'number.infinity': '{{#label}} cannot be infinity',
    'number.integer': t`number.integer`,
    'number.less': '{{#label}} must be less than {{#limit}}',
    'number.max': t`number.max`,
    'number.min': t`number.min`,
...
export const getJoi = (t: Translate) => {
...
export const extractDefaults = <T>(schema: Joi.ObjectSchema<T>) => {
  return schema.validate({}, { abortEarly: false }).value
}
